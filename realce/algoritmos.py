...
def aplicar_ecualizacion_global(imagen_gris: np.ndarray) -> np.ndarray:
    """
    ...
    Args:
        imagen_gris (np.ndarray): Imagen bidimensional de entrada en escala de grises (8 bits).
        
    Returns:
        np.ndarray: Nueva matriz con el histograma global ecualizado.
    """
...
def aplicar_clahe(imagen_gris: np.ndarray, clip_limit: float = 2.0, tile_grid_size: tuple = (8, 8)) -> np.ndarray:
    """
    ...
    Args:
        imagen_gris (np.ndarray): Imagen bidimensional de entrada en escala de grises.
        clip_limit (float): Umbral de recorte para el contraste. Valores típicos 
            oscilan entre 2.0 (sutil) y 4.0 (agresivo).
        tile_grid_size (tuple): Dimensiones de la matriz de bloques (filas, columnas).
            
    Returns:
        np.ndarray: Nueva matriz procesada con ecualización adaptativa.
    """
    if len(imagen_gris.shape) != 2:
        raise ValueError("La imagen de entrada debe estar en escala de grises (2 dimensiones).")
